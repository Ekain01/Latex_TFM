...
Para lograr un sistema robusto, modular y replicable, se ha diseñado una arquitectura basada en microservicios contenerizados, apoyada en las siguientes capas lógicas:
...
Las reglas de entrada del grupo de seguridad (Security Group) de la instancia EC2 en AWS definen qué tráfico de red está permitido acceder a la instancia. Estas reglas especifican los puertos, protocolos y orígenes que pueden comunicarse con el servidor. A continuación se presenta la configuración de las reglas implementadas en la solución:
...
primero se realiza la conexión mediante SSH a la instancia EC2 utilizando un cliente como PuTTY o la terminal de comandos, autenticándose con la clave privada asociada a la instancia. Una vez dentro del entorno de la instancia, se procede a instalar Docker con la siguiente secuencia de comandos:\par
\begin{codeframe}
\begin{lstlisting}[language=bash, caption={Comandos para la instalación de Docker en la instancia EC2.}]
    sudo apt update
...
El archivo docker-compose.yml utilizado para el despliegue de la arquitectura IIoT en AWS se muestra a continuación:
\begin{codeframe}
\begin{lstlisting}[language=bash, caption={Archivo Docker Compose para el despliegue de la arquitectura IIoT en AWS.}]
    version: '3.8'
...
\end{lstlisting}
\end{codeframe}
Para levantar la infraestructura, se ejecuta el siguiente comando en la terminal de la instancia EC2:
\begin{codeframe}
\begin{lstlisting}[language=bash, caption={Comando para iniciar los servicios definidos en el archivo Docker Compose.}]
