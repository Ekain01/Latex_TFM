...
\begin{table}[tb]
    ...
    \begin{tabular}{@{}clll@{}}
        ...
        % Unidad 001
        \multirow{2}{*}{\textbf{Unidad 001}} & Nivel 1: PD & \texttt{WTG1} & Identificador de hardware (PLC de la base) \\
                                             & Nivel 2: LD  & \texttt{WTG1} & Contenedor virtual de software (coincidente) \\ \bottomrule
...
\begin{table}[tb]
    ...
    \begin{tabular}{@{}clll@{}}
        ...
        \textbf{1 + 2} & Dispositivo Físico + Lógico & \texttt{WTG57} & Aerogenerador 57 (Identificador único) \\
...
Una vez definida la arquitectura semántica para los aerogeneradores bajo el estándar IEC 61400-25, se procede a estructurar las variables correspondientes a la subestación colectora del parque eólico. Para este dominio, se implementa de forma directa la norma matriz \textbf{IEC 61850}, manteniendo una homogeneidad absoluta en la filosofía de diseño orientada a objetos (Dispositivo Físico, Dispositivo Lógico, Nodo Lógico, Objeto de Datos y Atributo de Datos).

A diferencia de las variables de proceso analógicas o de los estados lógicos simples mapeados en las turbinas, la supervisión de la aparamenta de corte y maniobra de alta y media tensión (nodos lógicos \texttt{XCBR} y \texttt{XSWI}) exige un tratamiento de datos mucho más estricto para garantizar la seguridad operativa. Por ello, la norma prescribe el uso de un nuevo tipo de CDC denominada \textbf{DPS (\textit{Double Point Status})}.

La CDC DPS no utiliza un bit booleano convencional para determinar la posición de un equipo, sino que recurre a un código físico de doble punto (2 bits), cuyas combinaciones permiten descartar estados críticos del sistema de potencia como posiciones intermedias o estados de fallo eléctrico (Tabla \ref{tab:cdc_dps_atributos}).
...
\begin{table}[tb]
    ...
    \begin{tabular}{@{}llp{4.5cm}p{6cm}@{}}
        ...
        % Atributos de la CDC
        \texttt{DPS} & \texttt{stVal} & Codificación de 2 bits (\textit{DbPp}) & Valor de estado de posición real (00 01 10 11)\\
        \texttt{DPS} & \texttt{q}     & Calidad (\textit{Quality}) & Calidad de la señal (parámetros de validez e integridad del dato). \\
        \texttt{DPS} & \texttt{t}     & Marca de tiempo (\textit{Timestamp}) & Sello de tiempo exacto de la última transición de estado. \\ \bottomrule
    \end{tabular}
\end{table}

A diferencia de los aerogeneradores, donde la unificación de los niveles 1 y 2 se justificaba por compartir ubicación , en la subestación electrica se mantiene la separación entre el PD y LD.\par

En este caso el PD se corresponde con la celda de línea o acoplamiento, mientras que el LD se corresponde con el dispositivo lógico de control (\texttt{CTRL}) o el dispositivo lógico de protección (\texttt{PROT}) dependiendo de la función del nodo lógico.\par

Tras definir la estructura simplificada de cuatro niveles para los aerogeneradores, se aborda el modelado de la subestación colectora del parque eólico. En este dominio, las exigencias de operación multifabricante, interoperabilidad y segregación de funciones críticas obligan a respetar de forma estricta la arquitectura conceptual de cinco niveles prescrita por la norma internacional \textbf{IEC 61850} \cite{inproceedings}.

A diferencia de las turbinas, donde el hardware y el software se unifican de forma centralizada en un único PLC, la subestación presenta un entorno físicamente desacoplado. Por un lado, se sitúan los elementos de corte y maniobra de las celdas de línea o acoplamiento y, por otro, los activos de transformación de potencia con su instrumentación intrínseca de salvaguarda. Para absorber esta dualidad sin generar redundancias en la base de datos de \textit{SIMATIC WinCC V8.1}, se han diseñado dos tipos de estructura abstracts o moldes (\textit{Structure Types}), cuyas identidades funcionales se detallan en la Tabla \ref{tab:nomenclatura_estructuras_wincc}.
...
\begin{table}[t]
    ...
    \begin{tabular}{@{}llll@{}}
        \toprule
        \textbf{Tipo de Estructura} & \textbf{Estándar Base} & \textbf{Descripción Operativa del Bloque} & \textbf{Prefijo de Instancia Típico} \\ \midrule
        \texttt{CONTR} & IEC 61850 & Control de maniobra y medida de bahía (\texttt{CTRL}) & \texttt{SE.Qxx.CTRL} \\
        \texttt{PROT} & IEC 61850 & Protección y diagnóstico de transformador (\texttt{PROT}) & \texttt{SE.Txx.PROT} \\ \bottomrule
    \end{tabular}
\end{table}

\subsubsection{Estructuración Interna de los Tipos de Objeto y Nodos Lógicos}

La flexibilidad del SCADA radica en la instanciación selectiva de estos moldes según la tipología de celda física que se configure en el parque de intemperie:

\begin{enumerate}[leftmargin=*]
    \item \textbf{Celdas de Maniobra y Conexión (\texttt{SUB\_CTR}):} Asociadas al dispositivo lógico de control (\texttt{CTRL}). Su topología interna responde siempre a un esquema unifilar de tres equipos de corte en serie flanqueados por una unidad de medida. Mapea de forma nativa los nodos lógicos del seccionador de barra (\texttt{XSWI1}), el interruptor automático principal (\texttt{XCBR1}), el seccionador de línea (\texttt{XSWI2}) y el nodo de telemedida eléctrica (\texttt{MMXU1}).
    \item \textbf{Celda del Transformador de Potencia (\texttt{SUB\_TRF}):} Vinculada exclusivamente al dispositivo lógico de protección y monitorización de salud del activo (\texttt{PROT}). Reúne los nodos del transformador de potencia (\texttt{YPTR1}) para variables térmicas, la unidad de medida específica del trafo (\texttt{MMXU2}), el nodo de supervisión del aislamiento líquido y relé Buchholz (\texttt{SIML1}) y un nodo de entradas/salidas genéricas (\texttt{GGIO1}) encargado de absorber alarmas analógicas o digitales secundarias de la cuba.
\end{enumerate}

Esta arquitectura jerárquica de datos se consolida en el árbol de variables del proyecto a través de la concatenación sintáctica indexada por puntos, tal como se desglosa orientativamente en la Tabla \ref{tab:arbol_variables_subestacion}.
\begin{table}[tb]
    \centering
    ...
    \end{tabular}
\end{table}
\subsubsection{Mapeo de Variables y Mecánica de Instanciación en WinCC}

La declaración de las variables en el \textit{Tag Management} se automatiza vinculando la ruta de la instancia física con su correspondiente tipo de estructura de datos. De este modo, la sintaxis final de los \textit{Tags} que se comunican a través del servidor OPC UA de \textit{KepServerEX} se resuelve bajo la siguiente ecuación de direccionamiento:

\begin{equation}
\text{Prefijo de Celda (Instancia)} + \text{.} + \text{Miembro de Estructura} = \text{Tag Estructurado de WinCC}
\end{equation}

Bajo esta metodología, cuando el sistema requiere supervisar el estado mecánico del interruptor de la celda de línea \texttt{Q01}, WinCC genera automáticamente la variable interna \texttt{SE.Q01.CTRL.XCBR1.Pos.stVal}. Por su parte, la lectura de la temperatura del devanado del transformador se resuelve mediante la dirección \texttt{SE.T01.PROT.YPTR1.Tmp.mag}.

Este diseño modular y desacoplado garantiza dos ventajas críticas de ingeniería para el SCADA: en primer lugar, permite la reutilización de ventanas emergentes (\textit{Faceplates}) mediante el uso dinámico de prefijos de variables (\textit{Tag Prefix}) apuntando directamente a \texttt{CTRL.} o \texttt{PROT.}; en segundo lugar, confiere al sistema la flexibilidad necesaria para modificar, sustituir o ampliar los activos físicos de la subestación sin alterar la lógica de direccionamiento base ni la jerarquía del resto de posiciones operativas de la planta.
